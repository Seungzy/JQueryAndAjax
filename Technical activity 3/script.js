...
function renderProductDetails(p) {
  ...
  ['category','brand','sku','availabilityStatus','warrantyInformation','shippingInformation','returnPolicy'].forEach(k => {
    ...
    if (raw !== undefined && raw !== null && String(raw).trim() !== '') {
      if (k === 'returnPolicy') {
        // show just the friendly value for return policy
  ...
  if (p.meta) {
    ...
    if (p.meta.qrCode) {
      const qrCell = $('<td>');
      qrCell.append($('<a>').attr('href', p.meta.qrCode).attr('target', '_blank').attr('rel', 'noopener').text('View QR Code'));
...
function loadProducts(limit) {
  ...
  return $.ajax({
    ...
    method: 'GET',
    dataType: 'json',
    data: { limit: limit || 30, skip: 0 },
    timeout: 15000
  }).done(res => {

    allProducts = Array.isArray(res.products) ? res.products : [];
    setStatus(`Loaded ${allProducts.length} products (API total: ${res.total}). Click a product to view details.`);
    ...
    // Fallback to the sample data (so the page still demonstrates the full UI offline)
...
function wireEvents() {
  ...
  $('#limitSelect').on('change', function() {
    selectedId = null;
    loadProducts(Number($(this).val()));
  });
  $('#reloadBtn').on('click', function() {
    selectedId = null;
    loadProducts(Number($('#limitSelect').val()));
  });
  ...
  $('#toggleRawBtn').on('click', function() {
    ...
    $(this).attr('aria-pressed', String(showingRaw));
    $(this).text(showingRaw ? 'Hide raw JSON' : 'Show raw JSON');
    $('#rawJson').attr('hidden', !showingRaw);
  });
}
